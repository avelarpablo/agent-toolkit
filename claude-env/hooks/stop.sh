#!/bin/bash
# claude-env/hooks/stop.sh — Fires at end of every Claude Code session.
...
# Ship the schema next to the hook so the component is self-contained and both
# accounts read the same file (it used to live in the personal config dir).
SCHEMA="${CLAUDE_REVIEW_SCHEMA:-$(cd "$(dirname "$0")" && pwd)/review-schema.json}"
